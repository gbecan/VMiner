...
public class ClusteringSimDesc extends JPanel {
	...
	public ClusteringSimDesc(Map<DescCouple, Double> similarityMap,
		...
		while (iterator.hasNext()) {
			...
			if (weight >= 0.40) {
				...
				myAnalysisGraph.setEdgeWeight(myAnalysisGraph.getEdge(source, destination), similarityMap.get(key));
			}
		}
		...
		/*
		 * Clusters_Weight_Sup3 clusters = new Clusters_Weight_Sup3(); Product
		 * p1 =new Product("P1"); clusters.addClusters(p1, cliques);
		 * Collection<Set<Node>> cliques3
		 * =clusters.getCliques_weight_sup3().get(p1); for (Set<Node> clique :
		 * cliques3){
		 */
		System.out.println("clique3:" + cliques);

		updateCsvFile(
				"/home/user/git phd/familiar-language/FAMILIAR/R statistics new/descTrans.csv",
				descMap);
		CsvFileByCluster(descTrans, cliques);

		/*
		 * final JGraphSimpleLayout graphLayout = new
		 * JGraphSimpleLayout(JGraphSimpleLayout.TYPE_RANDOM, 100, 100); final
		 * JGraphFacade graphFacade = new JGraphFacade(myGraph);
		 * graphLayout.run(graphFacade); final Map nestedMap =
		 * graphFacade.createNestedMap(true, true);
	...
	private static void CsvFileByCluster(
		...
		for (Set<Node> clique : cliques) {
			if (clique.size() != 1) {
				...
				File dir = new File(
						"/home/user/git phd/familiar-language/FAMILIAR/R statistics new/");
				if (!dir.exists()) {
					dir.mkdir();
				}
				...
				// String fileUrl
				// ="/home/user/git/familiar-language/FAMILIAR/R statistics/cluster2."+k+".csv";
				csv.open(tagFile, ';');
	...
	private static void updateCsvFile(String fileUrl, Map<String, String> descMap)
			throws IOException {
			...
			for (Set<Node> clique : cliques) {
				...
				for (Node n : clique) {
					...
					if (n.getName().equals(descID.toString())) {
						...
						// clusters=clusters.concat(", "+clique.toString());
