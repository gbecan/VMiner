...
public class FRG extends JPanel {
...
*/
	// Noun
	// Pattern finalPat = Pattern.compile("[a-zA-Z0-9-.]+");
	/*
	 * Pattern pat3 = Pattern .compile("(expandable/JJ)\\s+(to/TO)");
	 */
	public FRG(Map<FeatureCouple, Double> similarityMap,
			CustomStringList valFeatures) throws IOException,
		 ...
		 * 
		 * String [] featureTransArray = feature.split(" "); CustomStringList
		 * newFeatureTrans = new CustomStringList();
		 * 
		 * for(int i=0; i<featureTransArray.length;i++){
		 * newFeatureTrans.add(featureTransArray[i]); }
		 ...
		 * CustomStringList newFeatureTrans = new CustomStringList(); String
		 * taggedFeature = tagger.tagString(feature);
		 * 
		 * for (Pattern p : patternsList) { //
		 * System.out.println("\n"+p.toString()+"\n"); Matcher matcher =
		 * p.matcher(taggedFeature); String nounGroupsTag; String
		 * nounGroupsTrans = null; int numGrp = 0;
		 * 
		 * while (matcher.find()) { nounGroupsTag = matcher.group(); //
		 * System.out.println(nounGroups + "\n");
		 * newFeatureTags.add(nounGroupsTag);
		 * 
		 * for (int i = 0; i < patterns.length; i++) { nounGroupsTrans =
		 * nounGroupsTag.replaceAll(patterns[i], ""); nounGroupsTag =
		 * nounGroupsTrans; System.out.println(nounGroupsTrans);
		 * 
		 * } nounGroupsTrans = nounGroupsTrans.toLowerCase(); nounGroupsTag =
		 * nounGroupsTag.trim(); nounGroupsTrans = nounGroupsTrans.trim();
		 * stopwords.setStopWords("stoplists/en.txt"); Set<String> stopList =
		 * stopwords.getStopWords(); if (!stopList.contains(nounGroupsTrans)) {
		 * if (!newFeatureTrans.contains(nounGroupsTrans)) {
		 * newFeatureTrans.add(nounGroupsTrans); } }
		 * 
		 * } }
		 * 
		 * featureTags.put(feature, newFeatureTags); featureTrans.put(feature,
		 * newFeatureTrans);
		 * 
		 * }
		 */
		...
		System.out.println("cliqTrans" + featureTrans + "\n");*/
		
		Lemmatizer lemmatizer = new Lemmatizer();
		String nounGroupsTag, nounPartTag;
		...
		for (String feature : valFeatures) {
			...
			for (Pattern p : patternsList) {
				...
				while (matcher.find()) {
					...
					//lemmaTrans =match noun group part in nounGroupsTrans + lemmatize it + and replace it 
					for (Pattern patt : patternsLemma) {
						...
						while (mat.find()) {
							...
							for (int i = 0; i < patterns.length; i++) {
								...
								System.out.println(nounPartTrans);
							}
							System.out.println(nounPartTrans);
							...
							if(nounGroupsTrans.contains(nounPartTrans)){
								...
								String firstWord = PCMBotTest.printFirst(boundary, nounPartTrans);
							...
							if(lemmaPartTrans!=null && partTrans!=null){
		...
		File dir = new File(
				"/home/user/FinalSpace/VMiner/R statistics new/ValFeatures/");
		if (!dir.exists()) {
			dir.mkdir();
		}
		...
		generateCsvFile("/home/user/FinalSpace/VMiner/R statistics new/ValFeatures/featureTrans.csv");
		fileName = "cluster1";
		...
		generateItemsetsCsvFile(
				"/home/user/FinalSpace/VMiner/R statistics new/ValFeatures/cluster1.csv",
				featureTrans);
	...
	private void generateCsvFile(String fileUrl) throws FileNotFoundException,
		...
		csv.open(new File(
				"/home/user/FinalSpace/VMiner/R statistics new/ValFeatures/featureTrans.csv"));
