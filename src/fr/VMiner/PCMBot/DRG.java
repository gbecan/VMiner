...
public class DRG extends JPanel {
	...
	public DRG(Map<DescCouple, Double> similarityMap,
		...
		/*
		 * Clusters_Weight_Sup1 clusters = new Clusters_Weight_Sup1(); Product
		 * p1 =new Product("P1"); clusters.addClusters(p1, cliques);
		 * Collection<Set<Node>> cliques1
		 * =clusters.getCliques_weight_sup1().get(p1);
		 */
		...
		while (it.hasNext()) {
			...
			// System.out
			// .println("*************New tagged requirement***************");
			String cle = (String) it.next();
			// System.out
			// .println(cle+": "+reqMap.get(cle));
			// The tagged string
			String taggedDesc = tagger.tagString(descMap.get(cle));

			// Output the result
			// System.out.println(taggedReq);
